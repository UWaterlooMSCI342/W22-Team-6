...
class FilterFeedbacksIndexFormTest < ApplicationSystemTestCase
  ...
  def test_filter_feedbacks_submitting_fields
    ...
    select "4", from: "participation_rating_start"
    select "5", from: "participation_rating_end"
    select "4", from: "effort_rating_start"
    select "5", from: "effort_rating_end"
    select "4", from: "punctuality_rating_start"
    ...
    assert_text "Participation Rating: 4-5"
    assert_text "Effort Rating: 4-5"
    assert_text "Punctuality Rating: 4-5"
  ...
  def test_one_rating_dropdown_selected
    visit root_url
    login 'msmucker@gmail.com', 'professor'
    ...
    assert_no_text "Applied filters:"

    # reset and select end only
    click_on "Feedback & Ratings"

    select "4", from: "participation_rating_end"
    select "4", from: "effort_rating_end"
    select "4", from: "punctuality_rating_end"
    click_on "Filter"
